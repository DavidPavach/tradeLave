...
export default function DepositRequestDetails({ request }: DepositRequestDetailsProps) {
    ...
    const handleSendUpdate = () => {
        ...
        const data: patchDepositDetails = { depositId: request._id, details: { role: "user", message: updateMessage.trim() } };
        if (hasPaid) {
            data.hasPaid = true;
        }
        ...
        updateRequest.mutate(data, {
            onSuccess: (response) => {
                toast.success(response.message || "Your bank transfer request was updated successfully!");
                reset();
            },

            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            onError: (error: any) => {
                const message = error?.response?.data?.message || "Failed to update your bank transfer request. Please try again.";
                toast.error(message);
                reset();
            },
        });
        ...
        <div className="space-y-6">
            ...
            {request.status !== "closed" && (
                <div className="pt-6 border-border border-t">
                    ...
                    <label className="block font-semibold">Send Update</label>
                    {!request.hasPaid &&
                        <div className="flex justify-between items-center mt-2">
                            <p>Click the card icon if you have paid 👉</p>
                            <CardTick1 onClick={handleHasPaid} variant="Bold" className="size-6 md:size-7 xl:size-8 text-green-600 dark:text-green-400 cursor-pointer" />
                        </div>
                    }
                    <div className="flex gap-2 mt-2">
                        <Textarea placeholder="Write your update here..." value={updateMessage} onChange={(e) => setUpdateMessage(e.target.value)} className="h-24 resize-none" disabled={updateRequest.isPending} />
                        <Button onClick={handleSendUpdate} disabled={!updateMessage.trim() || updateRequest.isPending} size="icon" className="h-24">
